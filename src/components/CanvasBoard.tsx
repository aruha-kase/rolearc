...
interface CanvasBoardProps {
  ...
  tempHiddenId?: string | null; // Cキー長押し中のオブジェクトID(他クライアントには完全透明で反映)
  onTempHide?: (objId: string | null) => void;
}
...
export const CanvasBoard = memo(function CanvasBoard({
  ...
  // Cキー長押し中だけ透明にする対象オブジェクトID
  const [keyHiddenId, setKeyHiddenId] = useState<string | null>(null);
  // Aキー長押し中: ホイールでオブジェクトをスケール
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      // Aキー長押し中はホイールで選択オブジェクトをスケール(盤面ズームの代わり)
      if (aHeldRef.current) {
        const objId = selectedObjectIdRef.current;
        const obj = objId ? objectsRef.current.find(o => o.id === objId) : undefined;
        if (objId && obj) {
          e.preventDefault();
          const scaleFactor = e.deltaY > 0 ? 0.94 : 1.06;
          const oldW = obj.width ?? 300;
          const oldH = obj.height ?? 300;
          const newW = Math.max(20, Math.min(3000, Math.round(oldW * scaleFactor)));
          const scale = newW / oldW;
          const updates: Record<string, unknown> = { width: newW, height: Math.round(oldH * scale) };
          const cropT = obj.crop_top ?? 0;
          const cropR = obj.crop_right ?? 0;
          const cropB = obj.crop_bottom ?? 0;
          const cropL = obj.crop_left ?? 0;
  ...
  }, [animateTransform, onUpdateObject]);
  ...
  // Aキー長押し検知(選択有無に関わらず常時監視。ホイールスケールで使用)
  useEffect(() => {
    if (!isAdmin && !enableKeyboardShortcuts) return;
    const handleADown = (e: KeyboardEvent) => {
    ...
    const handleAUp = (e: KeyboardEvent) => {
      if (e.key === 'a' || e.key === 'A') aHeldRef.current = false;
    };
    const handleABlur = () => { aHeldRef.current = false; };
    window.addEventListener('keydown', handleADown);
    window.addEventListener('keyup', handleAUp);
    window.addEventListener('blur', handleABlur);
    return () => {
      window.removeEventListener('keydown', handleADown);
      window.removeEventListener('keyup', handleAUp);
      window.removeEventListener('blur', handleABlur);
    };
  }, [isAdmin, enableKeyboardShortcuts]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'c':
        case 'C': {
          if (e.ctrlKey || e.metaKey) return; // Ctrl+C(コピー)は除外
          e.preventDefault();
          if (keyHiddenId !== selectedObjectId) {
            setKeyHiddenId(selectedObjectId); // 自分の画面では半透明表示
            onTempHide?.(selectedObjectId); // 他クライアントには完全透明として配信(キーリピートで連発しないようガード)
          }
          break;
        }
        case 'Escape':
    ...
    // ウィンドウ切り替え等でkeyupを取りこぼした場合の固着防止
    const handleBlur = () => {
      setKeyHiddenId(null);
      onTempHide?.(null);
    };
  ...
  }, [isAdmin, enableKeyboardShortcuts, selectedObjectId, objects, onUpdateObject, onDeleteObject, onSelectObject, onDragMove, keyHiddenId, onTempHide]);

  const renderableObjects = objects.filter(o => o.is_visible !== false || o.type !== 'video' || isEmbedLikeObject(o));

  // Cキー長押し: 自分の画面は半透明、他クライアント(配信された側)は完全透明
  const getTempOpacityOverride = useCallback((objId: string): number | undefined => {
    if (keyHiddenId === objId) return 0.35;
    if (tempHiddenId === objId) return 0;
    return undefined;
  }, [keyHiddenId, tempHiddenId]);
            ...
            >
              {sceneObjects.map(obj => (
                <DraggableObject
                  ...
                  opacityOverride={getTempOpacityOverride(obj.id)}
